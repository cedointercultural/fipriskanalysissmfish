...
# 
fipinventory <- read_googlesheet(sheetlink, sheetname = "inventario_fips", coltypes = "iccccccddccccci")
#col names are fip_id_number	fip_name	region	species	common_name_en	common_name	project_start_end_dates	estimated_total_fip_landings_mt	estimated_total_fishery_landings_mt	landings_date	organization_name	includes_climate_change	climate_change_requirement	proponent	fip_id_number
...
localities <- read_googlesheet(sheetlink, sheetname = "localidades_combinadas", coltypes = c("ciiiiccccc"))
...
lifehistorydata <- read_googlesheet(sheetlink, sheetname = "num_datos_caracteristicas_fip", coltypes = ("ccccccccccccccccccccccccciiiccccccccccccc"))
...
comunidadescosteras <- readr::read_csv(here::here("data-raw","comunidades_costeras_INEGI.csv"))
usethis::use_data(comunidadescosteras, overwrite = TRUE)
#creates template describing the data
data.description <-sinew::makeOxygen(comunidadescosteras)
cat(data.description, file=here::here("R",paste0("data-","comunidadescosteras.R")))

comunidadesedoscosteros <- readr::read_csv(here::here("data-raw","edos_costeros_INEGI.csv"))
usethis::use_data(comunidadesedoscosteros, overwrite = TRUE)
#creates template describing the data
data.description <-sinew::makeOxygen(comunidadesedoscosteros)
cat(data.description, file=here::here("R",paste0("data-","comunidadesedoscosteros.R")))

adaptivecapacitydata <- readr::read_csv(here::here("data-raw","adaptive-capacity_data.csv"))
...
sensitivitydata <- readr::read_csv(here::here("data-raw","sensitivity_data.csv"))
...
exposurehistorical <- readr::read_csv(here::here("data-raw","exposure_historical.csv"))
...
exposuressp126 <- readr::read_csv(here::here("data-raw","exposure_ssp126.csv"))
...
comunidades.coords.proj <- inegi.edo.coste %>% 
  dplyr::filter(COM_ID %in% fiplocal.id) %>% 
  dplyr::select(COM_ID, ENTIDAD, NOM_ENT, MUN, NOM_MUN, LOC, NOM_LOC, lon_deg, lon_min, lon_sec, lat_deg, lat_min, lat_sec, POBTOT) 

comunidades.coords.proj %>% 
  st_write(here("outputs","shp_files","comunidades_costeras_FIPs.shp"), append = FALSE)

 inegi.edo.coste %>% 
  dplyr::filter(COM_ID %in% fiplocal.id) %>% 
   dplyr::select(-PCDISC_MOT2) %>% 
  st_write(here("outputs","shp_files","inegi_FIPs.shp"), append = FALSE)

comunidades.coords.proj %>% 
  st_write("/home/atlantis/vulnerabilidadbidcomcost/inputs/shp_files/comunidades_costeras_FIPs.shp", append = FALSE)

 inegi.edo.coste %>% 
  dplyr::filter(COM_ID %in% fiplocal.id) %>% 
  dplyr::select(-PCDISC_MOT2) %>%  
  st_write("/home/atlantis/vulnerabilidadbidcomcost/inputs/shp_files/inegi_FIPs.shp", append = FALSE)

cost.utm.buff <- st_buffer(comunidades.coords.proj, dist=10000)

comunidades.edos <- comunidades.coords.proj %>% 
  st_drop_geometry()

readr::write_csv(comunidades.edos, here::here("outputs","comunidades_costeras_FIPs.csv"))

readr::write_csv(comunidades.edos, "/home/atlantis/vulnerabilidadbidcomcost/outputs/comunidades_costeras_FIPs.csv")

#check for localities missing data
loc.missing <- fiplocal.data %>% 
  dplyr::left_join(comunidades.edos, by="COM_ID") %>% 
  dplyr::filter(is.na(NOM_ENT)) %>% 
  dplyr::distinct(COM_ID, locality, municipality, state)

readr::write_csv(loc.missing, here::here("outputs","missing_locs.csv"))
...
pobreza <- c("prop_sin_escolar", "prop_sin_salud","prop_hog_sbienes","prop_desocup","irs","prop_alim")
...
loc.fip.vul <- localities %>%
  dplyr::select(COM_ID, fip_id_number) %>% 
  ...
  dplyr::group_by(fip_id_number, indicator) %>% 
...
#NOT RUN NEED SP DISTRIBUTIONS FEB/2024
